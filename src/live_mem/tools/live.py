...
def register(mcp: FastMCP) -> int:
    ...
    async def live_note(
        ...
        category: Annotated[
            str,
            Field(
                description="Catégorie : observation|decision|todo|insight|question|progress|issue"
            ),
        ],
        ...
        tags: Annotated[
            ...
            Field(
                default="",
                description="Tags séparés par des virgules (ex: 'auth,security,urgent')",
            ),
        ] = "",
    ) -> dict:
        """
    ...
    async def live_read(
        ...
        limit: Annotated[
            int,
            Field(
                default=50, description="Nombre max de notes à retourner (défaut 50)"
            ),
        ] = 50,
        category: Annotated[
            str,
            Field(
                default="",
                description="Filtrer par catégorie : observation|decision|todo|insight|question|progress|issue",
            ),
        ] = "",
        agent: Annotated[
            str, Field(default="", description="Filtrer par identifiant d'agent")
        ] = "",
        since: Annotated[
            str,
            Field(
                default="",
                description="Notes après cette date ISO 8601 (ex: '2026-03-08T10:00:00')",
            ),
        ] = "",
    ) -> dict:
        """
    ...
    async def live_search(
        space_id: Annotated[str, Field(description="Identifiant de l'espace cible")],
        query: Annotated[
            str, Field(description="Texte à chercher dans les notes (case-insensitive)")
        ],
        limit: Annotated[
            int, Field(default=20, description="Nombre max de résultats (défaut 20)")
        ] = 20,
    ) -> dict:
        """
